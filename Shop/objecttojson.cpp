...
QJsonObject ObjectToJson::integrateMessage(QJsonObject &object, Message *message)
{
    if (!message) return object;

    object.insert("messageId", message->getMessageId());
    object.insert("messageShopId", message->getMessageShopId());
    object.insert("messageContent", message->getMessageContent());
    object.insert("messageTime", message->getMessageTime().toString(Qt::ISODate));
    object.insert("readStatus", message->getReadStatus());
    object.insert("messageClientId", message->getMessageClientId());
    object.insert("messageClientName", message->getMessageClientName());
    object.insert("messageShopName", message->getMessageShopName());
    object.insert("messageFrom", message->getMessageFrom());

    return object;
}

QJsonObject ObjectToJson::integrateMessageList(QJsonObject &object, QList<Message*> messageList)
{
    QJsonArray array;
    for (Message *message : messageList) {
        if (message) {
            QJsonObject messageObject;
            integrateMessage(messageObject, message); // 调用单个转换函数
            array.append(messageObject);
        }
    }
    object.insert("MessageList", array);
    return object;
}

Message* ObjectToJson::parseMessage(const QJsonObject &object)
{
    Message *message = new Message();
    if (object.contains("messageId"))
        message->setMessageId(object.value("messageId").toInt());
    if (object.contains("messageShopId"))
        message->setMessageShopId(object.value("messageShopId").toInt());
    if (object.contains("messageContent"))
        message->setMessageContent(object.value("messageContent").toString());
    if (object.contains("messageTime"))
        message->setMessageTime(QDateTime::fromString(object.value("messageTime").toString(), Qt::ISODate));
    if (object.contains("readStatus"))
        message->setReadStatus(object.value("readStatus").toInt());
    if (object.contains("messageClientId"))
        message->setMessageClientId(object.value("messageClientId").toInt());
    if (object.contains("messageClientName"))
        message->setMessageClientName(object.value("messageClientName").toString());
    if (object.contains("messageShopName"))
        message->setMessageShopName(object.value("messageShopName").toString());
    if (object.contains("messageFrom"))
        message->setMessageFrom(object.value("messageFrom").toInt());

    return message;
}


QList<Message*> ObjectToJson::parseMessageList(QByteArray byteArray)
{
    QJsonParseError jsonError;
    ...
    if (!document.isNull() && jsonError.error == QJsonParseError::NoError) {
        if (document.isObject()) {
            ...
            if (object.contains("MessageList")) {
                QJsonValue value = object.value("MessageList");
                if (value.isArray()) {
                    QJsonArray array = value.toArray();
                    for (int i = 0; i < array.size(); i++) {
                        if (array[i].isObject()) {
                            QJsonObject messageObject = array[i].toObject();
                            Message *message = parseMessage(messageObject); // 调用单个解析函数
                            if (message) {
